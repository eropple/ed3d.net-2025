...
import {
  ...
  parsePrivateKey,
  SECP256K1_JWT_ALG,
} from "./atproto-crypto.js";
import { loginAgent, type LoginCredentials } from "./login.js";
...
export async function plcSetupLabeler(options: PlcSetupLabelerOptions) {
  ...
  const credentials: XRPCResponse<ComAtprotoIdentityGetRecommendedDidCredentials.Output> =
    await agent.get("com.atproto.identity.getRecommendedDidCredentials", {});
...
export async function plcClearLabeler(options: PlcClearLabelerOptions) {
  ...
  const credentials = await agent.get(
