...
export async function verifyJwt(
  ...
  const [_, rawPayload, sig] = jwtStr.split(".");
  if (!rawPayload || !sig) {
  ...
  const msgBytes = ui8.fromString(
    // TODO: efficient-ize this
    jwtStr.split(".").slice(0, 2).join("."),
    "utf8",
  );
  ...
  const signingKey = await resolveDidToSigningKey(
    ...
    logger.error({ err }, "Could not resolve DID");
  ...
  if (!validSig) {
    ...
    const freshSigningKey = await resolveDidToSigningKey(
      payload.iss,
      fetch,
      true,
    );
