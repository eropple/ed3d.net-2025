...
export const refreshGoogleTokensCommand = command({
  ...
  handler: async () => {
    ...
    try {
      const handle = await temporal.start("core", coreWorkflows.refreshGoogleTokensWorkflow, []);

      logger.info({ workflowId: handle.workflowId }, "Successfully started workflow");

    } catch (err) {
      logger.error({ err }, "Failed to trigger Google token refresh workflow");
      process.exit(1);
    } finally {
